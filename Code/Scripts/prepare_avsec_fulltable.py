...
def fulco_crispri_sample_generator_factory(fasta_extractor):
    for _, row in df.iterrows():
        ...
        metadata_dict = {
            ...
            "fulco_key": row["fulco_key"],
            "chromosome": row["chromosome"],
            "gene": row["Gene"],
            "enhancer_wide_type": args.enh_wide_type,
        }

        # first get the enhancer
        if args.enh_wide_type == "avsec":
            enh_start = row["enhancer_wide_start"]
            enh_end = row["enhancer_wide_end"]
        elif args.enh_wide_type == "fixed":
            enh_start = row["fix_enhancer_wide_start"]
            enh_end = row["fix_enhancer_wide_end"]
...
# * 2 (rev_comp) * 3 (offset) * N (shuffles + 1 none + 1 replace_with_n)
num_of_samples = len(df.index) * 2 * 3 * (args.shuffles + 2)
