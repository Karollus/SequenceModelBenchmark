...
batch_size = 16 if get_gpu_memory()[0] < 45000 else 32
...
def basenji2_batcherator(sample_generator, track_dict, batch_size):
    results = []
    done = False
    batch_idx = 0
    while True:
...
results = basenji2_batcherator(sample_generator,
...
for result in results:
    ...
    for track in result["predictions"]:
        ...
        row[track+"_center_sum"] = \
            np.sum(result["predictions"][track][510:514])
    rows.append(row)
